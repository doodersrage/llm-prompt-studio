"""Vendored Qwen Image ControlNet × img2img pipeline.

Diffusers 0.39 ships QwenImageControlNetPipeline (txt2img) and
QwenImageImg2ImgPipeline separately, but no classical
QwenImageControlNetImg2ImgPipeline. This file grafts img2img latent init
(strength / noisy init latents) onto the ControlNet denoise loop.

Based on huggingface/diffusers qwenimage ControlNet + Img2Img pipelines.
"""
...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> from diffusers import QwenImageControlNetModel, QwenImageMultiControlNetModel, QwenImageControlNetImg2ImgPipeline

        >>> # QwenImageControlNetModel
        >>> controlnet = QwenImageControlNetModel.from_pretrained(
        ...     "InstantX/Qwen-Image-ControlNet-Union", torch_dtype=torch.bfloat16
        ... )
        >>> pipe = QwenImageControlNetImg2ImgPipeline.from_pretrained(
...
class QwenImageControlNetImg2ImgPipeline(DiffusionPipeline, QwenImageLoraLoaderMixin):
    r"""
    ...
    def __call__(
        ...
        # 4. Preprocess init image for img2img
        if image is None:
            raise ValueError("`image` is required for QwenImageControlNetImg2ImgPipeline.")
        ...
        # 5. Prepare timesteps (with strength)
        ...
        # 6. Prepare latent variables from init image
        num_channels_latents = self.transformer.config.in_channels // 4
