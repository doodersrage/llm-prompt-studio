...
- **`ImageScale` / `ResizeImage` / `ImageScaleToTotalPixels` on ref paths are
  allowed** (Studio Compose LoadImage → scale → VAEEncode → ReferenceLatent).
  Filename resolution walks through the scale node; Diffusers still does its
  own ref resize. `Note`,
...
- **SDXL IP-Adapter identity lock is native.** `IPAdapterModelLoader` →
  ...
  still works. Combines with ControlNet and with img2img/inpaint on SDXL.
