...
- Diffusers engine: native Canny ControlNet for SDXL + classic Flux + Qwen (local opencv preprocessing, no extra model downloads), with safetensors-header sniffing to auto-detect "Union" checkpoints (ControlNetUnionModel + control_mode), reject unsupported XLabs-style Flux ControlNets and DiffSynth-style Qwen ControlNet "model patches" up front instead of failing deep in a torch error, and reject the mask-conditioned Qwen ControlNet-Inpainting checkpoint variant (unverified pipeline signature) in favor of the plain Union/Canny one; native inpaint for Flux + Qwen (previously SDXL-only) — closes the most common Comfy-fallback reasons. Pose/depth ControlNet, Flux2-Klein ControlNet, Flux2-Klein inpaint, IP-Adapter/InstantID/PuLID, FaceDetailer, and video stay on ComfyUI.
