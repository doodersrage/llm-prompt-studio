...
- **`diffusers`** (optional) — **local stills only** (txt2img + limited native graph compile) via `/api/diffusers/*` → local FastAPI (`DIFFUSERS_API_URL`, default `http://127.0.0.1:8190`). Opt in from Settings or `PROMPT_ENGINE=diffusers`. Native stills cover SDXL/Flux/Qwen/Flux2-Klein inpaint, ControlNet (± stacks / img2img where noted), Qwen Image Edit (+ Compose ReferenceLatent, single-image Edit-Plus inpaint), Klein ReferenceLatent edit, FluxGuidance, IP-Adapter/InstantID (SDXL), enrich polish (Spandrel / ScaleBy / Blur / Sharpen), and common Studio passthrough nodes. **Hard non-goals (stay on ComfyUI):** Play film / video, PuLID, FaceDetailer, Klein ControlNet / strength img2img, Qwen Edit+ControlNet, multi-ref Edit-Plus inpaint, LatentUpscale multi-pass, SD3/Boogu/GGUF/UltimateSDUpscale, Dynamic VRAM. On 24GB, Qwen Lightning quality/speed remains Comfy’s strength; Diffusers Dynamic VRAM / bf16 parity stays a non-goal (parked, see services/diffusers-engine/README.md).
