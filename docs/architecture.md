...
- **`diffusers`** (optional) — **local stills only** (txt2img + limited native graph compile) via `/api/diffusers/*` → local FastAPI (`DIFFUSERS_API_URL`, default `http://127.0.0.1:8190`). Opt in from Settings or `PROMPT_ENGINE=diffusers`. Inpaint (SDXL/Flux/Qwen), ControlNet Canny/OpenPose/depth/lineart/softedge/normal/MLSD (SDXL + classic Flux: ±img2img/inpaint and stacked multi-ControlNet; SDXL also ± IP-Adapter; Qwen InstantX CN-Inpainting on inpaint graphs), SDXL IP-Adapter identity lock, and Final/Max enrich (Spandrel upscale + `ImageScaleBy`) compile natively; everything else — Play film, Flux2-Klein ControlNet/inpaint, InstantID/PuLID, FaceDetailer, Dynamic VRAM — still routes to ComfyUI or a cloud engine. On 24GB, Qwen Lightning quality/speed remains Comfy’s strength; Diffusers Dynamic VRAM / bf16 parity stays a non-goal (parked, see services/diffusers-engine/README.md).
