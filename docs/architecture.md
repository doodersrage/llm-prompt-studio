...
- **`diffusers`** (optional) — **local stills only** (txt2img + limited native graph compile) via `/api/diffusers/*` → local FastAPI (`DIFFUSERS_API_URL`, default `http://127.0.0.1:8190`). Opt in from Settings or `PROMPT_ENGINE=diffusers`. Inpaint (SDXL/Flux/Qwen) and Canny ControlNet (SDXL + classic Flux + Qwen Union checkpoints, local opencv preprocessing) compile natively; everything else — Play film, pose/depth ControlNet, Flux2-Klein ControlNet, IP-Adapter/InstantID/PuLID, FaceDetailer, Dynamic VRAM, specialty enrich — still routes to ComfyUI or a cloud engine. On 24GB, Qwen Lightning quality/speed remains Comfy’s strength; Diffusers Dynamic VRAM / bf16 parity stays a non-goal (parked, see services/diffusers-engine/README.md).
