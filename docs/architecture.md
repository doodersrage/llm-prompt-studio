...
- **`diffusers`** (optional stills sidecar) — **local stills only** (txt2img + limited native graph compile) via `/api/diffusers/*` → local FastAPI (`DIFFUSERS_API_URL`, default `http://127.0.0.1:8190`). Opt in from Settings or `PROMPT_ENGINE=diffusers`. Not the product bet — Play film stays on Comfy/cloud; further Diffusers parity beyond the documented stills surface is parked. Native stills cover SDXL/Flux/Qwen/Flux2-Klein inpaint, ControlNet (± stacks / img2img where noted), Qwen Image Edit (+ Compose ReferenceLatent, single-image Edit-Plus inpaint), Klein ReferenceLatent edit, FluxGuidance, IP-Adapter/InstantID (SDXL), and enrich polish (Spandrel / `ImageScaleBy` / Blur / Sharpen). Ref paths allow `ImageScale` / `ResizeImage` / `ImageScaleToTotalPixels` (Studio Compose & Qwen/Flux enrich megapixel scale before `VAEEncode` / `ReferenceLatent`). Common Studio UI nodes (`Note` / `MarkdownNote` / `Reroute` / `LoadImageOutput` / `LoraLoader|pysssss`, etc.) passthrough. When Diffusers-first queues fall through, Studio tags `workflowSource=comfy-fallback` and surfaces `diffusersFallbackReason` in toast/status (`formatDiffusersQueueRouting`). **Hard non-goals (stay on ComfyUI):** Play film / video, PuLID, FaceDetailer, Klein ControlNet / strength img2img, Qwen Edit+ControlNet, multi-ref Edit-Plus inpaint, LatentUpscale multi-pass, SD3/Boogu/GGUF/UltimateSDUpscale, Dynamic VRAM / AIMDO. On 24GB, Qwen Lightning quality/speed remains Comfy’s strength; Diffusers Dynamic VRAM parity stays a non-goal (parked, see `services/diffusers-engine/README.md`).
